...
export class ScatterplotBuilder {
  public async apply(req: Request, res: Response) {
    ...
    try {
      params = this.parseParams(req.query);
    } catch (e) {
    ...
    const dataTable = this.buildDataTable(data, params);
    const response = this.buildResponse(dataTable, data);

    return res.json(response);
  }

  public parseParams(query: ParsedQs): ScatterplotParams {
    const errors = [];
  ...
  async queryData(params: ScatterplotParams): Promise<Issue[]> {
    ...
    if (params.excludeOutliers) {
      issues = excludeOutliers(issues, (issue: Issue) => issue.cycleTime);
    }
  ...
  public buildResponse(dataTable: DataTableBuilder, data: Issue[]) {
    return {
      meta: {
        issuesCount: data.length,
      },
      chartOpts: this.getChartOps(data),
      chartData: dataTable.build(),
    };
  }

  private getChartOps(data: Issue[]) {
    return {
      seriesType: "scatter",
